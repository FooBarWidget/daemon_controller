...
1. Graceful termination request: send SIGTERM to the PID. Then wait until daemon is gone.
   - No possibility to customize signal here. Rationale: this is an abnormal stop so we don't use the stop command.
2. Force termination upon timeout (`start_abort_timeout`): send SIGKILL to the PID and wait until daemon is gone, then delete the PID file.
   - No timeout here: we assume the OS processes it quickly enough.
